...
| ¬      | Negation      | stem:[\neg P]              | "not P"
| ∧      | Conjunction   | stem:[P \land Q]           | "P and Q"
| ∨      | Disjunction   | stem:[P \lor Q]            | "P or Q" (inclusive)
| →      | Implication   | stem:[P \to Q]             | "Whenever P then Q"
| ↔      | Biconditional | stem:[P \leftrightarrow Q] | "P if and only if Q" (iff)
|===
...
Example of a proposition: stem:[2 + 2 = 4]
...
| ℕ | Natural Numbers | stem:[\{0, 1, 2, 3, \ldots\}] -- Counting numbers.
| ℤ | Integers | stem:[\{\ldots, -2, -1, 0, 1, 2, \ldots\}] -- Whole numbers and their negatives.
| ℚ | Rational Numbers | Numbers that can be expressed as stem:[a/b], where stem:[a, b \in \mathbb{Z}], stem:[b \neq 0].
| ℝ | Real Numbers | All rational and irrational numbers.
| ℂ | Complex Numbers | stem:[a + bi], where stem:[i = \sqrt{-1}].
| ℍ | Quaternions | stem:[a + bi + cj + dk] -- Extension of complex numbers.
...
| 𝕋 | Transcendental Numbers | Numbers not roots of any polynomial with integer coefficients (e.g. stem:[\pi], stem:[e]).
...
| ∃ | Existential Quantifier | "There exists". Example: stem:[\exists x \in \mathbb{R}: x^2 = 4 \Rightarrow \text{true}].
| ∃! | Uniqueness Quantifier | "There exists exactly one". Example: stem:[\exists! x: x + 1 = 2]
| ∀ | Universal Quantifier | "For all". Example: stem:[\forall x \in \mathbb{N}, x \geq 0]
| ∈ | Element of | stem:[x \in A]: x is a member of set A.
| ∉ | Not an Element of | stem:[x \notin A]: x is not in A.
| ⊂ | Proper Subset | stem:[A \subset B]: all A in B, stem:[A \neq B].
| ⊆ | Subset | stem:[A \subseteq B]: all A in B, possibly stem:[A = B].
| ⊄ | Not a Subset | stem:[A \not\subset B]: A is not subset of B.
| ⊃ | Proper Superset | stem:[A \supset B]: A has all elements of B and more.
| ⊇ | Superset | stem:[A \supseteq B]: A contains B, possibly equal.
| ∪ | Union | stem:[A \cup B]: Set of elements in A, B, or both.
| ∩ | Intersection | stem:[A \cap B]: Elements common to A and B.
| \ | Set Difference | stem:[A \setminus B]: Elements in A not in B.
...
stem:[(A \land B) \Rightarrow A]

stem:[A \Rightarrow (A \lor B)]
...
- It depends on the interpretation of symbols like stem:[0] and stem:[S(x)], the _successor function_.
...
- *If* stem:[a = b] *and* stem:[b = c] , *then* stem:[a = c]

Addition Axiom: Equal quantities added to equal quantities yield equal sums.

- *If* stem:[a = b] *and* stem:[c = d] , *then* stem:[a + c = b + d]
